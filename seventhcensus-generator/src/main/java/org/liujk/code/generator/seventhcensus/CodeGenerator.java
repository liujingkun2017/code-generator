...
package org.liujk.code.generator.seventhcensus;
...
import org.liujk.code.generator.seventhcensus.config.*;
import org.liujk.code.generator.seventhcensus.config.query.SpringBootPlusMySqlQuery;
import org.liujk.code.generator.seventhcensus.config.query.SpringBootPlusSqlServerQuery;
import org.liujk.code.generator.seventhcensus.constant.GeneratorConstant;
import org.liujk.code.generator.seventhcensus.exception.GeneratorException;
import org.liujk.code.generator.seventhcensus.properties.GeneratorProperties;
import lombok.Data;
...
public class CodeGenerator {
    ...
    public void init(String tableName, GeneratorProperties generatorProperties) {
        ...
        if (StringUtils.isNotBlank(parentPackage)) {
            generatorProperties
                    .setParentPackagePath(parentPackage.replaceAll("\\.", Matcher.quoteReplacement(File.separator)));
        }
    ...
    public void generator(String tableName, String pkIdName, GeneratorProperties generatorProperties) {
        ...
        if (!CollectionUtils.isEmpty(tablePrefixList)) {
        ...
        InjectionConfig injectionConfig = new InjectionConfig() {
            ...
            public void initMap() {
                ...
                // 查询参数类路径
                map.put("pageParamClass",
                        pageParamPackage + StringPool.DOT + entityName + GeneratorConstant.PAGE_PARAM);
                // 查询参数共公包路径
        ...
        if (generatorConfig.isGeneratorMapperXml()) {
            focList.add(new FileOutConfig(GeneratorConstant.MAPPER_XML_TEMPLATE_PATH) {
                ...
                public String outputFile(TableInfo tableInfo) {
                    // 指定Mapper XML文件路径
                    return finalMapperXmlOutputDir + finalModulePath + File.separator + tableInfo.getEntityName()
                            + GeneratorConstant.MAPPER + StringPool.DOT_XML;
                }
            });
        }
        ...
        if (generatorConfig.isGeneratorPageParam()) {
            focList.add(new FileOutConfig(GeneratorConstant.PAGE_PARAM_TEMPLATE_PATH) {
                @Override
                public String outputFile(TableInfo tableInfo) {
                    return finalJavaOutputDir + File.separator + parentPackagePath + finalModulePath + File.separator
                            + GeneratorConstant.PARAM + File.separator + tableInfo.getEntityName()
                            + GeneratorConstant.PAGE_PARAM + StringPool.DOT_JAVA;
                }
            });
        }
        ...
        if (generatorConfig.isGeneratorQueryVo()) {
            focList.add(new FileOutConfig(GeneratorConstant.QUERY_VO_TEMPLATE_PATH) {
                ...
                public String outputFile(TableInfo tableInfo) {
                    return finalJavaOutputDir + File.separator + parentPackagePath + finalModulePath + File.separator
                            + GeneratorConstant.VO + File.separator + tableInfo.getEntityName()
                            + GeneratorConstant.QUERY_VO + StringPool.DOT_JAVA;
                }
            });
        }
